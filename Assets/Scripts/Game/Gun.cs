...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            var firstAngle = (shotgunBullets / 2) * shotgunSpread;
            var firstRotation = director.rotation.eulerAngles;
            firstRotation.y -= firstAngle;

            for(int i = 0; i<shotgunBullets; ++i)
            {
                var bulletRotation = firstRotation;
                bulletRotation.y += shotgunSpread * i;
                SpawnBullet(director.position, Quaternion.Euler(bulletRotation));
            }
        }
        else
        {
